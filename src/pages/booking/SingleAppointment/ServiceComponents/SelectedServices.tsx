    ...
    return (
        <>
            <div className="w-full h-full">
                ...
                <div className=" w-full main-tabs px-[20px]">

                    {bookings.map((booking, i) => (
                        <SelectedServiceCard key={i} showDialog={showDialogForService} booking={booking} />
                    ))}
                </div>
                </div>
            </div>

            <SelectProfessionalForServiceDialog service={selectedService} open={open} handleClose={setOpen} />
        </>
    );
}
