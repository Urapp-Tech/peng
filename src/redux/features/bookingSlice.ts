...
export const bookingSlice = createSlice({
  ...
  reducers: {
    ...
    addBarberOnSpecificService: (state, action: PayloadAction<{service: StoreService, barber: Barber}>) => {
    ...
    addAnyBarberOnSpecificService: (state, action: PayloadAction<{service: StoreService}>) => {
      state.bookings = state.bookings.map(x => {
        if(action.payload.service.id  == x.service.id) {
          x.barber = undefined;
        }
...
export const { addService, removeService, addBarberToAll, removeBarberFromAll, addAnyBarberOnSpecificService, addBarberOnSpecificService } = bookingSlice.actions;
