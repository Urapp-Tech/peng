...
const GroupSelectProfessionalForServiceDialog: React.FC<
  GroupSelectProfessionalForServiceDialogProps
> = ({ service, handleClose, open = false }) => {
  const dispatch = useAppDispatch();
  const { bookings, selectedCustomer } = useAppSelector(
  ...
  useEffect(() => {
    const b = barbers.filter(
      (barber) =>
        bookings.filter(
          (x) => x.customer !== selectedCustomer && x.barber?.id === barber.id
        ).length === 0
    );
    setFilterBarbers(b);
  }, [barbers]);
  ...
  useEffect(() => {
    if (bookings.length > 0) {
      dispatch(
        fetchCBarber(
          bookings.reduce((uniqueIds: string[], b: GroupBooking) => {
