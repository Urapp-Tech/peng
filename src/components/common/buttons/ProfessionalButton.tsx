...
const ProfessionalButton = ( {onclick, barber}:Props) => {
  return (
    <>
        <Button className="bg-white border-2 border-primary rounded-[32px] px-[10px] py-[25px]" onClick={onclick}>
          <div className="flex justify-between items-center p-[5px]">
            <span className="block">
              <img
                src={barber ? barber.store_employee.avatar : assets.images.avatar}
                alt="icon"
                width="29px"
                height="29px"
            ...
            <span className="block px-[10px] text-heading-color text-[14px] leading-normal font-bold">
              {barber ? barber.store_employee.name : 'Any Professional'}
            </span>
            <span className="block">
              <svg
